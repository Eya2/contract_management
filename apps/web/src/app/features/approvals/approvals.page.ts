...
@Component({
  ...
  template: `
    ...
    @for (s of queue.value(); track s.id) {
      <article class="mb-4 rounded-xl bg-white p-5 shadow-sm ring-1 ring-slate-200" [class.!ring-orange-300]="s.overdue">
        <div class="flex flex-wrap items-start gap-4">
          <div class="min-w-0 flex-1">
            <div class="flex flex-wrap items-center gap-2">
              <a [routerLink]="['/contracts', s.request.contract.id]" [queryParams]="{ tab: 'approvals' }" class="text-lg font-semibold text-ink hover:text-brand-700">
                {{ s.request.contract.title }}
              </a>
              @if (s.overdue) {
                <span class="rounded bg-orange-100 px-1.5 py-0.5 text-xs font-medium text-orange-700">Overdue</span>
              }
              @if (s.escalatedToMe) {
            ...
            <p class="mt-3 text-sm">
              ...
              <span class="text-slate-500"> (stage {{ s.stage }}) · requested by {{ fullName(s.request.submittedBy) }} · {{ dateTime(s.request.submittedAt) }}</span>
            </p>
            @if (s.dueAt) {
              <p class="text-sm" [class]="s.overdue ? 'text-orange-700' : 'text-slate-500'">Due {{ dateTime(s.dueAt) }}</p>
            }
            @if (s.routingNote) {
          ...
          <div class="text-right">
            ...
            <div class="mt-3 flex gap-2">
              <button mat-stroked-button class="!text-rose-700" (click)="reject(s)">Reject</button>
