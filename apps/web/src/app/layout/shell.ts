    ...
    <div class="min-h-screen lg:pl-[17rem]">
      ...
      >
        <div class="flex h-16 items-center justify-between px-5">
          ...
          <button mat-icon-button class="lg:!hidden" (click)="drawerOpen.set(false)" aria-label="Close menu"><mat-icon>close</mat-icon></button>
        </div>

        @if (auth.can('contract.create')) {
          <div class="px-4 pb-2">
            <a mat-flat-button routerLink="/contracts/new" class="!w-full" (click)="drawerOpen.set(false)"><mat-icon>add</mat-icon>New contract</a>
          </div>
        }
        ...
        <nav class="flex-1 space-y-6 overflow-y-auto px-3 py-4">
          @for (group of groups(); track group.label) {
            <div>
              <p class="mb-1.5 px-3 text-[11px] font-semibold tracking-[0.12em] text-faint uppercase">{{ group.label }}</p>
        ...
        <div class="space-y-3 border-t border-line p-4">
          ...
          <mat-menu #userMenu="matMenu" yPosition="above">
            ...
            <mat-divider />
            <a mat-menu-item routerLink="/settings" (click)="drawerOpen.set(false)"><mat-icon>manage_accounts</mat-icon>Account settings</a>
            <a mat-menu-item routerLink="/notifications" (click)="drawerOpen.set(false)"><mat-icon>notifications</mat-icon>Notifications</a>
            <mat-divider />
      ...
      <header class="sticky top-0 z-20 border-b border-line/70 bg-canvas/80 backdrop-blur-xl">
        <div class="mx-auto flex h-16 max-w-7xl items-center gap-3 px-4 sm:px-6 lg:px-8">
          <button mat-icon-button class="lg:!hidden" (click)="drawerOpen.set(true)" aria-label="Open menu"><mat-icon>menu</mat-icon></button>
          ...
          <form class="relative ml-auto w-full max-w-md lg:ml-0" (ngSubmit)="search()" role="search">
            ...
            <input
              name="q"
              [(ngModel)]="query"
              placeholder="Search contracts, references, counterparties…"
